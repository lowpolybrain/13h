(function() {
    ...
    function addLoadListener(name, listener) {
      console.log('added load listener', name, listener);
    ...
    function resolve(name, value) {
        console.log('Resolved', name, value);
    ...
    window.require = function(deps, definition) {
        console.log('Required', deps, definition);
        if (deps.length === 0) {
    ...
    window.define = function(name, deps, definition) {
        console.log('Defined', name, deps, definition);
        if (!definition) {

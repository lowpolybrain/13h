import { animate, color, point, random, Point, TWO_PI } from '@13h/core';
...
const getPos = (i: number, n: number): Point => {
  ...
  const distr = ((n / speed + random.m32(i + count)) % 0.9) + 0.1;
  const dist = canvas.width * distr;
  const pos = point.add(point.rotate([dist, 0], rot), canvas.center);
  return pos;
};

...
preloadImages(
  ...
  ([sprite]) => {
    ...
    const canva = sprite.createCanvas(
      [0, 16],
      [16 * 6, 32]
    );
    setColorTransparency(
      canva,
      getColor(canva, [12, 14]),
      0.5
    );
    ...
    const animationBounce = new FlyingBounce(
      FlyingBounce.randomDirection(5),
      16 * as,
      canvas.size,
      canvas.center
    );
    ...
    const bounces = new Array(40).fill(0).map(() => {
      ...
      return new FlyingBounce<string>(
        point.rotate([0.001, 0], rot),
        0.005
      )
        .setPayload(color.hslRot(rot))
        .rewind(400);
    });
    ...
    animate((n: number) => {
      ...
      animation
        .setSpriteIndex(
          Math.floor(n / 10) % animation.lastFrameIndex
        )
